...
@class RTC_OBJC_TYPE(RTCRTPHeaderData);
...
/** gets the CSRC details from the peerconnection */
- (void)getRTPHeaderPacketsWithCompletionHandler:(void (^)(uint32_t sequenceNumber, uint32_t timeStamp, uint32_t rtpTimeStamp, uint32_t SSRC, uint32_t CSRC))completionHandler;
